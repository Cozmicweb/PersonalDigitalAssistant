...
Items are mapped to "display handlers" in `data/pda/data_maps/item/display_handlers.json`. Display handlers extend the abstract class `cozmicweb.pda.common.display.handlers.InfoDisplayHandler` and usually override the `#getDisplayText()` method. Default handlers are registered in `cozmicweb.pda.common.display.InfoDisplayManager` with the `#register(Identifier, Class)` method from the common side `@Mod` constructor. The `Identifier` provided when registering is used in `display_handlers.json` to map display handlers to items.
