...
public class BattleStandardItem extends Item {
    ...
    private static InteractionResult revealing(Level level, Player player, BlockPos pos) {
        ...
        if (army != null) {
            ...
            if (level instanceof ServerLevel)
                player.sendSystemMessage(Component.translatable("text.pda.standard.success.revealed").withColor(TextColor.GOLD));
            return InteractionResult.SUCCESS;
        } else {
            if (level instanceof ServerLevel)
                player.sendSystemMessage(Component.translatable("text.pda.standard.fail.inactive_here").withColor(TextColor.RED));
    ...
    private static InteractionResult gathering(Level level, Player player, BlockPos pos) {
        ...
        if (army != null) {
            ...
            if (level instanceof ServerLevel)
                player.sendSystemMessage(Component.translatable("text.pda.standard.success.gathered", targetCount).withColor(TextColor.LIGHT_PURPLE));
            return InteractionResult.SUCCESS;
        } else {
            if (level instanceof ServerLevel)
                player.sendSystemMessage(Component.translatable("text.pda.standard.fail.inactive_here").withColor(TextColor.RED));
